...
class FormMapper extends BaseGroupedMapper
{
     ...
     * @param bool   $deleteTabWhenEmpty Wether or not the Tab should be deleted, when the deleted group leaves the tab emmpty after deletion
     */
    public function removeGroup($group, $tab = 'default', $deleteTabWhenEmpty = false)
    {
        ...
        if ($deleteTabWhenEmpty && count($tabs[$tab]['groups']) == 0) {
            unset($tabs[$tab]);
        }
